...
• Comment configurer tout ça sur un routeur et dans Packet Tracer`
      ...
      },
      {
        ...
        title: "Quiz : DHCP & DNS",
        questions: [
          { q: "Pourquoi utilise-t-on DHCP ?", options: ["Pour que les PC reçoivent leur adresse tout seuls (plus de config à la main)", "Pour aller plus vite sur internet", "Pour remplacer le routeur"], a: 0, explanation: "Le DHCP donne tout aux PC automatiquement : adresse, passerelle, DNS. Tu configures une fois sur le routeur." },
          { q: "Que signifie DORA ?", options: ["Un protocole de routage", "Les 4 étapes : PC demande, routeur propose, PC accepte, routeur confirme", "Un mot de passe"], a: 1, explanation: "D = le PC cherche une IP, O = le routeur propose, R = le PC accepte, A = le routeur valide." },
          { q: "Qu'est-ce que le DHCP donne aux PC ?", options: ["Juste l'adresse IP", "Uniquement le masque", "L'adresse, la passerelle, le serveur DNS et le masque"], a: 2, explanation: "Il donne tout : l'IP, le masque, la passerelle (la sortie) et l'adresse du serveur DNS." },
          { q: "À quoi sert le DNS ?", options: ["À traduire les noms (google.fr, intranet) en adresses", "À rien", "À remplacer le DHCP"], a: 0, explanation: "Comme un annuaire : tu donnes un nom, il te donne l'adresse. Tu n'as pas à mémoriser les chiffres." },
          { q: "Peut-on configurer un serveur DNS dans Packet Tracer ?", options: ["Non", "Seulement sur un routeur", "Oui : serveur → Services → DNS → ajouter des fiches nom → adresse"], a: 2, explanation: "Tu cliques sur le serveur, tu vas dans Services → DNS, et tu ajoutes des enregistrements." },
          { q: "Quelle commande réserve des adresses pour qu'elles ne soient pas données aux PC ?", options: ["ip dhcp excluded-address", "exclude-address", "dhcp exclude"], a: 0, explanation: "ip dhcp excluded-address réserve une plage (par ex. la passerelle et le serveur DNS)." },
          { q: "Pourquoi doit-on configurer 'ip domain-name' avant 'crypto key generate rsa' pour activer SSH ?", options: ["Ce n'est pas obligatoire", "Les clés RSA utilisent hostname + domaine pour générer l'identité cryptographique de l'équipement", "Pour accélérer la connexion"], a: 1, explanation: "Cisco construit l'identité (certificat) à partir du hostname et du domain-name. Sans eux, la génération de clés échoue." },
          { q: "Où est stockée la configuration après 'copy running-config startup-config' ?", options: ["En RAM (perdue au redémarrage)", "En NVRAM (persistante)", "Sur un serveur TFTP"], a: 1, explanation: "running-config = RAM (actuelle) ; startup-config = NVRAM. La copy sauvegarde la config pour qu'elle survive au redémarrage." },
          { q: "Que signifie 'line vty 0 4' sur un routeur Cisco ?", options: ["5 lignes virtuelles (0 à 4) pour connexions à distance — donc 5 sessions Telnet/SSH simultanées possibles", "Une seule ligne pour la console", "Désactive les connexions à distance"], a: 0, explanation: "vty = virtual terminal. 0 4 = 5 lignes (0, 1, 2, 3, 4). Chaque ligne peut accepter une connexion Telnet ou SSH." },
          { q: "Pour que 'login local' fonctionne sur line vty, que faut-il avoir configuré avant ?", options: ["Uniquement transport input ssh", "Au moins un 'username ... secret' (ou password) en config globale", "Uniquement ip domain-name"], a: 1, explanation: "login local = authentification via les utilisateurs définis par 'username'. Sans username, personne ne peut se connecter en SSH/Telnet." }
        ]
      },
      {
        type: 'command_builder',
