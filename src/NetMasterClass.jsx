...
• Comment configurer tout ça sur un routeur et dans Packet Tracer`
      ...
      },
      {
        ...
        questions: [
          ...
          { q: "Quel prompt indique le mode privilégié sur un routeur Cisco ?", options: [">", "#", "(config)#"], a: 1, explanation: "Le # apparaît après la commande enable. Le > est le mode utilisateur, (config)# le mode configuration." },
          { q: "Que fait la commande 'copy running-config startup-config' ?", options: ["Redémarre le routeur", "Sauvegarde la config active dans la NVRAM (persistante après redémarrage)", "Efface la configuration"], a: 1, explanation: "Elle copie la config en RAM vers la NVRAM. Sans ça, les modifs sont perdues au redémarrage." },
          { q: "Pourquoi utiliser 'enable secret' plutôt que 'enable password' ?", options: ["enable secret est plus rapide", "enable password ne fonctionne pas", "enable secret est chiffré (hashé), donc plus sécurisé"], a: 2, explanation: "enable secret est stocké de façon chiffrée ; enable password en clair. Toujours préférer enable secret." },
          { q: "Pour autoriser la connexion SSH sur les lignes VTY, on tape :", options: ["telnet on", "transport input ssh", "ssh enable"], a: 1, explanation: "transport input ssh impose SSH (au lieu de Telnet) pour les connexions à distance (line vty)." }
        ]
      },
      {
        type: 'command_builder',
